...
# Wofi launcher script with custom options
# Usage: ./wofi-launcher.sh

show_emoji_picker() {
  # Define emoji list with descriptions
  emojis="😀 grinning face face smile happy joy :D grin
...
• dot circle separator"
  ...
  # Use wofi to select emoji with same styling
  selected=$(echo "$emojis" | wofi --dmenu --prompt "Emojis" --width 600 --height 400)
  ...
  if [ -n "$selected" ]; then
    ...
    # Show notification
    notify-send -a "Emoji Picker" "Emoji Copied" "$emoji copied to clipboard" --expire-time=2000
  fi
}
...
# Launch wofi with custom options
selected=$(echo -e "$options" | wofi --dmenu --hide-search --width 400 --height 180)

case "$selected" in
"Applications")
  # Launch normal wofi drun
  wofi --show drun
  ;;
"Edit Config")
  # Open .config directorys with nvim
  kitty -e nvim ~/.config &
  ;;
"Emoji Picker")
  # Launch emoji picker seamlessly
  show_emoji_picker
  ;;
*)
  # If nothing selected or invalid option, launch normal drun
  if [ -n "$selected" ]; then
    wofi --show drun
  fi
  ;;
